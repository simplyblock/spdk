			 ...
			 struct spdk_io_channel *ch, struct spdk_nvmf_request *req)
{
	...
	if (spdk_unlikely(rc)) {
		...
		if (opts.memory_domain) {
			a = 1;
		}
		if (req->iov && req->iov[0].iov_base) {
			b = 1;
		}
		SPDK_ERRLOG("Unable to process the read IO %d  %d  %d .\n", rc, a , b);
		rsp->status.sct = SPDK_NVME_SCT_GENERIC;
			  ...
			  struct spdk_io_channel *ch, struct spdk_nvmf_request *req)
{
	...
	if (spdk_unlikely(rc)) {
		...
		SPDK_ERRLOG("Unable to process write IO %d.\n", rc);
		rsp->status.sct = SPDK_NVME_SCT_GENERIC;
		rsp->status.sc = SPDK_NVME_SC_INTERNAL_DEVICE_ERROR;
		return SPDK_NVMF_REQUEST_EXEC_STATUS_COMPLETE;
	}
			    ...
			    struct spdk_io_channel *ch, struct spdk_nvmf_request *req)
{
	...
	if (spdk_unlikely(rc)) {
		...
		SPDK_ERRLOG("Unable to process the comparev IO %d.\n", rc);
		rsp->status.sct = SPDK_NVME_SCT_GENERIC;
		rsp->status.sc = SPDK_NVME_SC_INTERNAL_DEVICE_ERROR;
		return SPDK_NVMF_REQUEST_EXEC_STATUS_COMPLETE;
	}
				      ...
				      struct spdk_io_channel *ch, struct spdk_nvmf_request *cmp_req, struct spdk_nvmf_request *write_req)
{
	...
	if (spdk_unlikely(rc)) {
		...
		SPDK_ERRLOG("Unable to process the compare and write IO %d .\n", rc);
		rsp->status.sct = SPDK_NVME_SCT_GENERIC;
		rsp->status.sc = SPDK_NVME_SC_INTERNAL_DEVICE_ERROR;
		return SPDK_NVMF_REQUEST_EXEC_STATUS_COMPLETE;
	}
				 ...
				 struct spdk_io_channel *ch, struct spdk_nvmf_request *req)
{
	...
	if (spdk_unlikely(rc)) {
		...
		SPDK_ERRLOG("Unable to process the write zero IO %d.\n", rc);
		rsp->status.sct = SPDK_NVME_SCT_GENERIC;
		rsp->status.sc = SPDK_NVME_SC_INTERNAL_DEVICE_ERROR;
		return SPDK_NVMF_REQUEST_EXEC_STATUS_COMPLETE;
	}
			  ...
			  struct spdk_io_channel *ch, struct spdk_nvmf_request *req)
{
	...
	if (spdk_unlikely(rc)) {
		...
		SPDK_ERRLOG("Unable to process the flush %d.\n", rc);
		      ...
		      struct nvmf_bdev_ctrlr_unmap *unmap_ctx)
{
	...
	if (unmap_ctx == NULL) {
		...
		if (!unmap_ctx) {
			SPDK_ERRLOG("Unable to process ctrlr unmap IO due to allocate ctx.\n");
			response->status.sc = SPDK_NVME_SC_INTERNAL_DEVICE_ERROR;
			return SPDK_NVMF_REQUEST_EXEC_STATUS_COMPLETE;
		}
	...
	for (i = unmap_ctx->range_index; i < nr; i++) {
		...
		if (rc) {
			...
			SPDK_ERRLOG("Unable to process ctrlr unmap IO %d.\n", rc);
			response->status.sc = SPDK_NVME_SC_INTERNAL_DEVICE_ERROR;
			unmap_ctx->count--;
			 ...
			 struct spdk_io_channel *ch, struct spdk_nvmf_request *req)
{
	...
	if (spdk_unlikely(rc)) {
		...
		SPDK_ERRLOG("Unable to process ctrlr copy IO %d.\n", rc);
		response->status.sct = SPDK_NVME_SCT_GENERIC;
		response->status.sc = SPDK_NVME_SC_INTERNAL_DEVICE_ERROR;
		return SPDK_NVMF_REQUEST_EXEC_STATUS_COMPLETE;
	}
		...
		spdk_nvmf_nvme_passthru_cmd_cb cb_fn)
{
	...
	if (spdk_unlikely(req->iovcnt > 1)) {
		SPDK_ERRLOG("Unable to process the passthru.\n");
		req->rsp->nvme_cpl.status.sct = SPDK_NVME_SCT_GENERIC;
	...
	if (spdk_unlikely(rc)) {
		...
		if (rc == -ENOTSUP) {
			...
			SPDK_ERRLOG("Unable to process the passthrue ..  %d .\n", rc);
			req->rsp->nvme_cpl.status.sc = SPDK_NVME_SC_INTERNAL_DEVICE_ERROR;
		}
			    ...
			    struct spdk_nvmf_request *req)
{
	...
	if (spdk_unlikely(rc != 0)) {
		...
		SPDK_ERRLOG("Unable to process the zcopy IO %d .\n", rc);
		rsp->status.sct = SPDK_NVME_SCT_GENERIC;
		rsp->status.sc = SPDK_NVME_SC_INTERNAL_DEVICE_ERROR;
		return SPDK_NVMF_REQUEST_EXEC_STATUS_COMPLETE;
	}
