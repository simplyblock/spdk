...
spdk_bs_drain_queued_io(struct spdk_io_channel_iter *i)
{
    ...
    /* iterate over captured list */
    for (int idx = 0; idx < ctx->count; idx++) {
        struct spdk_hublvol_channels *hub = &ctx->ch_list[idx];

        if (hub == NULL || hub->ch == NULL) {
            continue;
        }

        /* only operate on channels that belong to this thread */
...
void
spdk_bs_drain_channel_queued(struct spdk_blob_store *bs,
                             struct spdk_hublvol_channels *ch_list,
                             int count,
                             spdk_drain_op_cpl cb_fn,
                             void *cb_arg)
		...
		if (desc->type == SPDK_MD_DESCRIPTOR_TYPE_PADDING) {
				...
				for (j = 0; j < desc_extent_rle->extents[i].length; j++) {
					if (desc_extent_rle->extents[i].cluster_idx != 0) {
						if (blob->examine_flag) {
							if (!spdk_bit_pool_is_allocated(blob->bs->used_clusters, desc_extent_rle->extents[i].cluster_idx + j)) {
								spdk_bit_pool_allocate_specific_bit(blob->bs->used_clusters, desc_extent_rle->extents[i].cluster_idx + j);
								if (blob->bs->num_free_clusters > 0) {
									blob->bs->num_free_clusters--;
								}
							}
						} else if (!spdk_bit_pool_is_allocated(blob->bs->used_clusters,
			...
			for (i = 0; i < extent_pages_length / sizeof(desc_extent_table->extent_page[0]); i++) {
				if (desc_extent_table->extent_page[i].page_idx != 0) {
					...
					if (blob->examine_flag) {
						spdk_bit_array_set(blob->bs->used_md_pages, desc_extent_table->extent_page[i].page_idx);
					}
				} else if (spdk_blob_is_thin_provisioned(blob)) {
				...
				if (desc_extent->cluster_idx[i] != 0) {
					...
					if (blob->examine_flag) {
						if (!spdk_bit_pool_is_allocated(blob->bs->used_clusters, desc_extent->cluster_idx[i])) {
							spdk_bit_pool_allocate_specific_bit(blob->bs->used_clusters, desc_extent->cluster_idx[i]);
							if (blob->bs->num_free_clusters > 0) {
								blob->bs->num_free_clusters--;
							}
						}
					} else if (!spdk_bit_pool_is_allocated(blob->bs->used_clusters, desc_extent->cluster_idx[i])) {
	   ...
	   struct spdk_blob *blob)
{
	...
	if (blob->examine_flag) {
		uint32_t	page_num = bs_blobid_to_page(blob->id);
		spdk_bit_array_set(blob->bs->used_md_pages, page_num);
	}
...
static void persist_bs_write_used_blobids(spdk_bs_sequence_t *seq, void *arg, int bserrno);
...
blob_persist_zero_pages_cpl(spdk_bs_sequence_t *seq, void *cb_arg, int bserrno)
{
	...
	if (delete_blob) {
		...
		ctx->bit_cb_fn_persist = blob_persist_clear_clusters;
...
persist_bs_write_used_blobids(spdk_bs_sequence_t *seq, void *arg, int bserrno)
{
	...
	uint64_t first_page_bits;
	uint64_t first_bit_inpage;
	uint64_t last_bit_inpage;
	uint64_t pageidx;
	uint64_t lba;
	...
	if (ctx->idx_blobids >= spdk_bit_array_capacity(bs->used_blobids)) {
		SPDK_ERRLOG("Blob index %" PRIu64" exceeds used-blob-ID bitmap capacity %u\n",
			ctx->idx_blobids, spdk_bit_array_capacity(bs->used_blobids));
		cb_fn(seq, ctx, -ERANGE);
		return;
	}
	...
	first_page_bits = SPDK_BS_PAGE_SIZE_INBIT - SPDK_BS_MD_STRUCT_INBIT;
	memset(ctx->bit_page, 0, SPDK_BS_PAGE_SIZE);

	if (ctx->idx_blobids < first_page_bits) {
		pageidx = 0;
		first_bit_inpage = 0;
		last_bit_inpage = first_page_bits;
		...
		last_bit_inpage = first_bit_inpage + SPDK_BS_PAGE_SIZE_INBIT;
	...
	if (pageidx >= bs->used_blobid_mask_len) {
		SPDK_ERRLOG("Bitmap page out of bounds: blob_idx=%" PRIu64", pageidx=%" PRIu64", "
			"mask_len=%u\n", ctx->idx_blobids, pageidx, bs->used_blobid_mask_len);
		cb_fn(seq, ctx, -ERANGE);
		return;
	}
	...
	bs->w_io++;
	bs_sequence_write_dev(seq, ctx->bit_page, lba, bs_byte_to_lba(bs, SPDK_BS_PAGE_SIZE), cb_fn, arg);
}
...
struct spdk_bs_load_ctx {
	...
	struct spdk_bs_load_ctx   *master_worker;
	bool				scan_done;
...
spdk_bs_monitoring_poller(void *cb_arg) {
	...
	if (bs && !bs->stop) {
		uint64_t ccm_io =  bs->r_io + bs->w_io;
		bs->total += ccm_io;
		bs->total_r += bs->r_io;
		bs->total_w += bs->w_io;
...
static bool
bs_load_examine_assign_next(struct spdk_bs_load_ctx *worker)
{
	struct spdk_bs_load_ctx *master = worker->master_worker;
	uint32_t page;

	page = spdk_bit_array_find_first_set(master->bs->used_blobids, master->cur_page);

	if (page >= spdk_bit_array_capacity(master->bs->used_blobids)) {
		master->scan_done = true;
		return false;
	}

	master->cur_page = page + 1;
	worker->page_index = page;

	return true;
}

static void
bs_load_blob_iter_on_examine(struct spdk_bs_load_ctx *ctx);

static void
bs_load_iter_on_examine_cpl(void *cb_arg, int bserrno)
{
	struct spdk_bs_load_ctx *worker = cb_arg;
	struct spdk_bs_load_ctx *master = worker->master_worker;
	...
	if (bserrno != 0 && bserrno != -ENOENT) {
		SPDK_ERRLOG("Error loading blob on examine: %d\n", bserrno);
	...
	/*
	 * Error: stop submitting new work and drain outstanding workers.
	 */
	if (master->rc != 0) {
		if (master->active_workers != 0) {
			return;
		}
	...
	/*
	 * No more blobs can be submitted.
...
finish:
	...
	master->iter_cb_fn = NULL;

	if (master->super) {
		spdk_free(master->super);
		master->super = NULL;
	}

	if (master->mask) {
		spdk_free(master->mask);
		master->mask = NULL;
	}

	bs_sequence_finish(master->seq, master->rc);
	free(master);
}
...
bs_delete_corrupted_blob_examine_cpl(void *cb_arg, int bserrno)
{
	...
	bs_load_iter_on_examine_cpl(ctx, 0);
	return;
}
...
static void
bs_load_iter_on_examine(void *arg, struct spdk_blob *blob, int bserrno)
{
	struct spdk_bs_load_ctx *ctx = arg;
	const void *value;
	size_t len;
	...
	if (bserrno == 0) {
		...
		/* Open clone to check if we are able to fix this blob or should we remove it */
		spdk_bs_open_blob_on_examine(ctx->bs, *(spdk_blob_id *)value, bs_examine_clone_without_close, ctx);
		return;
	} else if (bserrno == -ENOENT) {
		SPDK_INFOLOG(blob, "Opening blobs on examine done.\n");
		bs_load_iter_on_examine_cpl(ctx, -ENOENT);
		return;
	} else {
		/*
		 * This case needs to be looked at further.  Same problem
	...
	bs_load_iter_on_examine_cpl(ctx, bserrno);
}
...
bs_load_used_blobids_cpl_clean_mode(spdk_bs_sequence_t *seq, void *cb_arg, int bserrno)
{
	...
	spdk_bit_array_load_mask(ctx->bs->used_blobids, ctx->mask->mask);
	num_md_clusters = spdk_bit_array_count_set(ctx->used_clusters);
	ctx->bs->used_clusters = spdk_bit_pool_create_from_array(ctx->used_clusters);
...
bs_load_blob_iter_on_examine(struct spdk_bs_load_ctx *ctx)
{
	...
	assert(ctx->page_index < spdk_bit_array_capacity(bs->used_blobids));
...
bs_load_used_blobids_cpl_dump(spdk_bs_sequence_t *seq, void *cb_arg, int bserrno)
{
	...
	if (memcmp(ctx->mask->mask, mask->mask, comp_size)) {
		SPDK_NOTICELOG("Dumping blobid bitmap not match in the blobstore\n");
	}
	spdk_free(ctx->mask);
	spdk_free(mask);
...
static void
bs_load_read_used_blobid_page_dump(spdk_bs_sequence_t *seq, struct spdk_bs_load_ctx *ctx)
{
	uint64_t lba, lba_count, mask_size;
...
bs_dump_super_cpl(spdk_bs_sequence_t *seq, void *cb_arg, int bserrno)
{
	...
	if(ctx->dump_type) {
		...
		spdk_free(mask);
		bs_load_read_used_blobid_page_dump(seq, ctx);		
	} else {
	       ...
	       struct spdk_blob	*blob)
{
	...
	if (rc == 0) {
		...
		blob_set_xattr(tmp_blob, SNAPSHOT_PENDING_REMOVAL, (spdk_blob_id *)value,
...
bs_swap_blobs_repair_chains(struct spdk_blob *destblob, struct spdk_blob *srcblob, bool same_parent) {
	...
	if (same_parent) {
		...
		*
		* srcblob and destblob have the same parent, but blob loading creates a
		* new back_bs_dev context for srcblob. Creating that context does not take
		* an additional parent blob reference.
		*
		* Therefore, do not call back_bs_dev->destroy() here, since destroy may
		* release the parent blob. Only free the stale destblob back_bs_dev context;
		* srcblob->back_bs_dev will replace it below.
		*/
		if (destblob->back_bs_dev && !blob_backed_with_zeroes_dev(destblob)) {
			free(destblob->back_bs_dev);
		}
		destblob->back_bs_dev = NULL;
	} else {
		if (destblob->parent_id != SPDK_BLOBID_INVALID) {
			parentblob = blob_lookup(destblob->bs, destblob->parent_id);
			if (parentblob) {
				if (destblob->back_bs_dev && !blob_backed_with_zeroes_dev(destblob)) {
					if (parentblob->open_ref > 1) {
						destblob->back_bs_dev->destroy(destblob->back_bs_dev);
					} else {
						free(destblob->back_bs_dev);
					}
				}
				...
				snapshot_entry = bs_get_snapshot_entry(destblob->bs, parentblob->id);
				if (snapshot_entry) {
					TAILQ_FOREACH(clone_entry, &snapshot_entry->clones, link) {
						if (clone_entry->id == destblob->id) {
							find = true;
							break;
						}
					}
					...
					if (find && clone_entry) {
						...
						if (snapshot_entry->clone_count > 0) {
							...
							SPDK_ERRLOG("Snapshot 0x%" PRIx64
									" clone_count is zero while removing clone 0x%"PRIx64 "\n",
									snapshot_entry->id, destblob->id);
						}
						free(clone_entry);
					}

					if (parentblob->open_ref != snapshot_entry->clone_count + 1) {
	...
	if (srcblob->parent_id == SPDK_BLOBID_INVALID) {
		return;
	}

	new_parent_entry = bs_get_snapshot_entry(srcblob->bs, srcblob->parent_id);
	if (!new_parent_entry) {
		SPDK_ERRLOG("CRITICAL: parent snapshot 0x%" PRIx64" not found for source blob 0x%" PRIx64 "\n",
		    srcblob->parent_id, srcblob->id);
		return;
	}
...
bs_swap_blobs(struct spdk_blob *destblob, struct spdk_blob *srcblob)
{
	...
	bs_swap_blobs_repair_chains(destblob, srcblob, destblob->parent_id == srcblob->parent_id);
...
void
spdk_bs_get_snapshot_tree(struct spdk_blob_store *bs)
{
...
delete_snapshot_cleanup_clone(void *cb_arg, int bserrno)
{
	...
	if (ctx->clone->frozen_refcnt > 0) {
		blob_unfreeze_io(ctx->clone, delete_snapshot_cleanup_clone, ctx);
		return;
	}
...
delete_snapshot_sync_snapshot_cpl(void *cb_arg, int bserrno)
{
	...
	if (snapshot_entry) {
		...
		assert(snapshot_entry->clone_count == 0);
	}
	...
	default:
		...
		if (parent_snapshot_entry != NULL) {
			if (!clone_entry) {
				...
				parent_snapshot_entry->clone_count--;
			} else {
...
clone_update_delete_sync_cpl(void *cb_arg, int lvolerrno)
{
	...
	clone->state = SPDK_BLOB_STATE_CLEAN;
	if (lvolerrno < 0) {
		SPDK_ERRLOG("Could not update clone blob for sync lvol delete blobid 0x%" PRIx64 " \n", clone->id);
		return;
	}
	...
	if (ctx->parent_id != clone->parent_id && ctx->parent_id != SPDK_BLOBID_INVALID) {
		...
		parentblob = blob_lookup(clone->bs, ctx->parent_id);
		if (parentblob && snapshot_entry && parentblob->open_ref != snapshot_entry->clone_count + 1 ) {
			parentblob->open_ref = snapshot_entry->clone_count + 1;
		}
	}
...
static void
delete_blob_manually(struct spdk_blob_store *bs, struct spdk_blob *blob, bool single) {
	struct spdk_blob_list *snapshot_entry = NULL;
...
spdk_bs_delete_blob_non_leader(struct spdk_blob_store *bs, struct spdk_blob *blob) 
{
	...
	struct spdk_blob *removal_clone;
	bool corrupted_mode = false;
	const void *value;
	int rc;
	size_t len;
	...
	rc = blob_get_xattr_value(blob, SNAPSHOT_PENDING_REMOVAL, &value, &len, true);
	if (rc == 0) {
		removal_clone = blob_lookup(bs, *(spdk_blob_id *)value);
		if (removal_clone) {
			corrupted_mode = true;
			SPDK_NOTICELOG("Blob is a snapshot and was in delete before - corrupted snapblob in secondary mode 0x%" PRIx64 ".\n", blob->id);
		}
	}
	...
	if (update_clone || corrupted_mode) {
		...
		* snapshot that we are removing */
		...
		delete_blob_manually(bs, blob, false);
		    ...
		    spdk_blob_op_complete cb_fn, void *cb_arg)
{
	...
	seq = bs_sequence_start_bs(bs->md_channel, &cpl);
	if (!seq) {
		SPDK_ERRLOG("Cannot allocate seq delete async lvol.\n");
		cb_fn(cb_arg, -ENOMEM);
		return;
	}
	...
	ctx = calloc(1, sizeof(*ctx));
	if (ctx == NULL) {
		SPDK_ERRLOG("Cannot allocate ctx delete async lvol.\n");
		bs_sequence_finish(seq, -ENOMEM);
		return;
	}
	ctx->is_async = true;
	     ...
	     void *cb_arg)
{
	...
	page_num = bs_blobid_to_page(blobid);
	if (status != SPDK_BLOB_LOAD_EXAMINE && spdk_bit_array_get(bs->used_blobids, page_num) == false) {
		/* Invalid blobid */
		cb_fn(cb_arg, NULL, -ENOENT);
		return;
	}
	...
	if (status == SPDK_BLOB_LOAD_EXAMINE) {
		blob->examine_flag = true;
	}
...
static void
blob_free_cluster_msg_cpl(void *arg)
{
	struct spdk_blob_cluster_op_ctx *ctx = arg;
...
blob_free_cluster_msg_cb(void *arg, int bserrno)
{
	...
	ctx->rc = bserrno;
	spdk_thread_send_msg(ctx->thread, blob_free_cluster_msg_cpl, ctx);
}
...
static void
blob_free_cluster_clear_ep_cpl(void *arg, int bserrno)
{
	struct spdk_blob_cluster_op_ctx *ctx = arg;
...
static void
blob_write_zero_extent_page_cpl(spdk_bs_sequence_t *seq, void *cb_arg, int bserrno)
{
	bs_sequence_finish(seq, bserrno);
}

static void
blob_free_cluster_clear_ep_cb(void *arg, int bserrno)
{
	struct spdk_blob_cluster_op_ctx *ctx = arg;	
	struct spdk_blob		*blob = ctx->blob;
	struct spdk_blob_store		*bs = blob->bs;
	...
	seq = bs_sequence_start_bs(blob->bs->md_channel, &cpl);
	if (!seq) {
		blob_free_cluster_clear_ep_cpl(ctx, -ENOMEM);
		return;
	}
		       ...
		       struct spdk_blob_md_page *page, spdk_blob_op_complete cb_fn, void *cb_arg)
{
	...
	if (!bs_load_cur_extent_page_valid(page)) {
		printf("invalid extent page for new insert\n");
	}
...
blob_free_cluster_msg(void *arg)
{
	...
	tmp_cluster = ctx->blob->active.clusters[ctx->cluster_num];
	...
	if (free_extent_page) {
		...
		ctx->page = spdk_zmalloc(SPDK_BS_PAGE_SIZE, 0, NULL, SPDK_ENV_SOCKET_ID_ANY,
				    SPDK_MALLOC_DMA);
		if (ctx->page == NULL) {
			SPDK_ERRLOG("Failed to allocate memory for extent page in UNMAP IO\n");	 
			ctx->blob->active.clusters[ctx->cluster_num] = tmp_cluster;
			if (ctx->cluster != 0) {
				ctx->blob->active.num_allocated_clusters++;
			}
			blob_op_cluster_msg_cb(ctx, 0);
