...
bdev_nvme_destroy_bdev_channel_cb(void *io_device, void *ctx_buf)
{
	struct nvme_bdev_channel *nbdev_ch = ctx_buf;
	struct nvme_bdev *nbdev = io_device;
	SPDK_NOTICELOG("Destroying bdev channel %p for device: %s.\n", nbdev_ch, nbdev->disk.name);
	   ...
	   struct spdk_nvme_qpair *qpair, uint16_t cid)
{
	...
	SPDK_ERRLOG("CSTS_GET_START: nvme_ctrlr=%p spdk_ctrlr=%p qpair=%p qid=%u cid=%u type=%s "
		...
		nvme_ctrlr, ctrlr, qpair, qid, cid,
	...
	if (nvme_ctrlr->active_path_id->trid.trtype == SPDK_NVME_TRANSPORT_PCIE || qpair != NULL) {
		...
		if (csts.bits.cfs) {
			...
			SPDK_ERRLOG(
				"CSTS_GET_DONE: nvme_ctrlr=%p spdk_ctrlr=%p qpair=%p qid=%u cid=%u raw=0x%08x "
				"rdy=%u cfs=%u shst=%u thread=%s elapsed_us=%" PRIu64 "\n",
				nvme_ctrlr, ctrlr, qpair, qid, cid,
				csts.raw, csts.bits.rdy,
				csts.bits.cfs, csts.bits.shst,
