...
struct rpc_bdev_lvol_set_tiering_info {
	char* lvol_name;
...
static void 
free_rpc_bdev_lvol_set_tiering_info(struct rpc_bdev_lvol_set_tiering_info *req) {
	free(req->lvol_name);
}

static const struct spdk_json_object_decoder rpc_bdev_lvol_set_tiering_info_decoders[] = {
	{"lvol_name", offsetof(struct rpc_bdev_lvol_set_tiering_info, lvol_name), spdk_json_decode_string},
...
static void
rpc_bdev_lvol_set_tiering_info_cb(void *cb_arg, int lvolerrno)
{
	struct spdk_jsonrpc_request *request = cb_arg;
...
static void 
rpc_bdev_lvol_set_tiering_info(struct spdk_jsonrpc_request *request,
			      const struct spdk_json_val *params) 
{
	struct rpc_bdev_lvol_set_tiering_info req = {};
	struct spdk_lvol *lvol;
	struct spdk_bdev *lvol_bdev;

	SPDK_INFOLOG(lvol_rpc, "Set tiering info of lvol\n");

	if (spdk_json_decode_object(params, rpc_bdev_lvol_set_tiering_info_decoders,
				    SPDK_COUNTOF(rpc_bdev_lvol_set_tiering_info_decoders),
				    &req)) {
		SPDK_INFOLOG(lvol_rpc, "spdk_json_decode_object failed\n");
		spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
						 "spdk_json_decode_object failed");
...
cleanup:
	free_rpc_bdev_lvol_set_tiering_info(&req);
}
...
static void rpc_bdev_lvol_recover(struct spdk_jsonrpc_request *request,
			      const struct spdk_json_val *params)
{
...
struct rpc_bdev_lvol_set_persistent {
	char *lvol_name;
	bool should_persist;
};

static const struct spdk_json_object_decoder rpc_bdev_lvol_set_persistent_decoders[] = {
	{"lvol_name", offsetof(struct rpc_bdev_lvol_set_persistent, lvol_name), spdk_json_decode_string},
	{"should_persist", offsetof(struct rpc_bdev_lvol_set_persistent, should_persist), spdk_json_decode_bool}
};

static void rpc_bdev_lvol_set_persistent(struct spdk_jsonrpc_request *request,
			      const struct spdk_json_val *params)			  
{
	SPDK_INFOLOG(lvol, "Set whether an lvol's data and metadata should be persisted instead of wiped upon bdev removall\n");

	struct rpc_bdev_lvol_set_persistent req = {};
	if (spdk_json_decode_object(params, rpc_bdev_lvol_set_persistent_decoders,
				    SPDK_COUNTOF(rpc_bdev_lvol_set_persistent_decoders),
				    &req)) {
		SPDK_INFOLOG(lvol_rpc, "spdk_json_decode_object failed\n");
		spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
						 "spdk_json_decode_object failed");
	...
	vbdev_lvol_set_persistent(lvol, req.should_persist);
	spdk_jsonrpc_send_bool_response(request, true);

cleanup:
	free(req.lvol_name);
}
SPDK_RPC_REGISTER("bdev_lvol_set_persistent", rpc_bdev_lvol_set_persistent, SPDK_RPC_RUNTIME)